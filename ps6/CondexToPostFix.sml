...
and expToCmds (Condex.Int i) depth = [PostFix.Int i]
  ...
  | expToCmds (Condex.RelApp(relop,exp1,exp2)) depth =
    [PostFix.Int 17] (* replace this stub *)
  | expToCmds (Condex.Ifnz(testExp,thenExp,elseExp)) depth =
    [PostFix.Int 23] (* replace this stub *)
...
fun testCompiler (condexFileName, testCases) =
  let val _ = print ("-------------------------------------------------\n"
      ...
      val postfixPgm = condexToPostFix(condexPgm)
      val _ = print ("PostFix program output of compiler:\n"
...
val compilerTestCases = [
  ...
  ("lec30Test.cdx", ST [([5,8,0],"3"),([6,17,0],"5"),
			([0,~8,2],"~4"), ([0,~18,3],"~6")
			(* [2020/12/10, lyn] too hard to make Condex
                           and PostFix errors the same; ignore
			   these for now:
