...
fun sexpToPgm (Sexp.Seq[Sexp.Sym "condex", Sexp.Int n, body]) =
    Condex(n, sexpToExp body)
...
and sexpToExp (Sexp.Int i) = Int i
  ...
  | sexpToExp (Sexp.Seq[Sexp.Sym opname, rand1, rand2]) =
    if Utils.listMember opname ["<", "=", ">"] then
	RelApp(stringToRelop opname, sexpToExp rand1, sexpToExp rand2)
    else if Utils.listMember opname ["+", "-", "*", "/", "%"] then
	ArithApp(stringToArithop opname, sexpToExp rand1, sexpToExp rand2)
    else
	raise (SyntaxError ("invalid Condex operator: " ^ opname))
  | sexpToExp (Sexp.Seq[Sexp.Sym "ifnz", tst, thn, els]) =
    Ifnz(sexpToExp tst, sexpToExp thn, sexpToExp els)
...
val interpreterTestCases = [
  ...
  ("prob3a-g.cdx", IT [([10,2,8],17), ([11,2,8],~102), ([12,3,8],8), 
		     ([~7,2,3],38), ([5,4,5],~40)]),
  ("lec30Test.cdx", ST [([5,8,0],"3"),([6,17,0],"5"),
			([0,~8,2],"~4"), ([0,~18,3],"~6"),
			([2,1,0], "Eval error: Division by 0"),
			([0,1,2], "Eval error: Remainder by 0")])
]
